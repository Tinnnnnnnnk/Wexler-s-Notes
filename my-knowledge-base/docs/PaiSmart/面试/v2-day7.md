...
- MySQL 在这个项目里主要存用户、文件元数据、chunk元数据，不承担每次问答的核心相似度计算。读写压力有，但不是 RAG 在线问答的主计算瓶颈。
- Kafka 主要在上传合并后的异步解析链路里发挥作用，它影响的是文档入库速度，不直接影响用户每次问答的在线检索延迟。除非上传量暴涨、文档解析积压，否则它不是最先影响问答体验的瓶颈。
- 1：系统的QPS与用户请求量正相关
- 2：向量相似度的计算属于CPU密集型任务，结合全文检索后，IO、内存、CPU都会成为压力点
- 3：随着知识库文档数量的增长，ES索引膨胀，查询延迟会逐步增加
- 4：如果ES查询很慢，就会阻塞LLM提问链路，影响问答响应速度
- 我打算在下个版本中引入FAISS，FAISS支持高效的内存结构和向量压缩算法