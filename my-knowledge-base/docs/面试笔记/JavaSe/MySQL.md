...
#### 事务的隔离级别如何实现
##### 读未提交
- 读未提交通过行锁共享锁确保一个事务在更新行数据但没有提交的情况下，其他事务不能更新该行数据，但不会阻止脏读
##### 读已提交
- 读已提交会在更新数据前 加 行级排他锁，不允许其他事务写入或读取未提交的数据，也就是事务2不可以在事务1提交之前读取事务1修改的数据，解决了脏读
- 同时会在每次读取前生成一个新的 ReadView，导致不可重复读的问题
##### 可重复读
...
#### 幻读
- 幻读是指同一个事务下，多次执行相同的范围查询，结果不同。通常是因为其他事务在两次查询之间插入或删除了符合当前查询条件的数据
##### 如何避免幻读
- 在可重复读 隔离级别下，通过MVCC和临键锁一定程度避免幻读
- 比如查询时显示加锁，利用临建锁锁定查询范围，防止其他事务插入新数据，此时其他事务插入数据就会阻塞，直到当前事务提交或回滚
##### 快照读
- 快照读就是读取快照，例如`SELECT * FROM table`
- 不涉及到数据的修改，只要读取MVCC即可
##### 当前读
- 涉及到数据的修改，例如`update、delete`
- 读取记录的最新提交版本，并且读取的时候加锁，确保其他并发事务不能修改当前记录
#### MVCC
- 多版本并发控制，每次修改数据时，都会生成一个新的版本，而不是在原数据上修改，每个事务只能看到在它开始之间已经提交的数据版本
- 底层主要依赖于 undolog 和 Read View
- 每次修改数据前，会将记录拷贝到undo log，每条记录包含三个隐藏列去标记相关数据
- 每次读取数据时，在 RC 级别下每次读都会生成新的；在 RR 级别下仅在第一次读时生成，去记录当前活跃事务的相关数据，通过与undo log的数据对比，可以判断当前事务是否能看到该数据版本
### 高可用
#### Mysql的读写分离
- 读写分离就是把 写操作给主库，读操作分给多个从库处理
#### 主从复制原理
- 一种数据同步机制，将数据从主数据库复制到一个或多个从数据库
- 主库执行事务提交时，会将数据变更以事件形式记录到 binlog。从库通过IO线程从主库的binlog读取变更事件，并将事件写入本地的中继日志文件中，SQL线程会实时监控中继日志内容，顺序读取执行事件，保证从库主库数据一致
#### 主从同步延迟
- 主从同步的延迟是因为从库要先接受binlog，再执行SQL才能同步主库数据，在高并发写或网络抖动时容易出现延迟
- 一：对于一致性要求高的查询直接走主库
- 二：对于非关键业务允许短暂数据不一致，可以提示用户“数据同步中，请稍后刷新”，然后借助异步通知机制替代实时查询。
- 三：采用半同步复制，主库在事务提交时，要等至少一个从库确认收到 binlog（但不要求执行完成），才算提交成功。
